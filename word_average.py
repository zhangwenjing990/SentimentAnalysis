...
class CustomAverageModel(nn.Module):
    ...
    def forward(self, inputs, mask):
        ...
        embed_average = (embedded * mask.unsqueeze(-1)).sum(1) / torch.sum(mask, dim=-1, keepdim=True)
...
def padding(batch):
    ...
    return padded_inputs, inputs_mask, torch.tensor(targets, dtype=torch.float32)
...
def train():
    ...
    for epoch in range(cfg.max_epoch):
        ...
        if min_count == cfg.early_stop:
            break
        model_path = cfg.model_save_path + 'epoch_' + str(epoch)
...
def accuracy(data, model):
    # model = CustomAverageModel(cfg).to(cfg.device)
    # model.load_state_dict(torch.load('./output/epoch_100', map_location='cpu'))
    model.eval()
...
def text_inference():
    model = CustomAverageModel(cfg).to(cfg.device)
    model.load_state_dict(torch.load('./output/epoch_154', map_location='cpu'))
    model.eval()
...
def analysis():
    ...
    scores = torch.matmul(embedding_weights, output_weights.transpose(1, 0)).abs()
