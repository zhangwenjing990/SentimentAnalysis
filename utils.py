...
def load_vocab(vocab_path):
    ...
    # 本模型使用字符级别的token方式，因此将词表缩减为单个字符

    special_token = ["[PAD]", "[UNK]", "[CLS]", "[SEP]"]
    ...
    for t in special_token:
        ...
        keep_tokens.append(char2idx[t])

    for t, _ in sorted(char2idx.items(), key=lambda s: s[1]):
...
# def load_pretrained_bert(model, pretrained_model_path, keep_tokens=None):
#     checkpoint = torch.load(pretrained_model_path)
#     # checkpoint = {k: v for k, v in checkpoint.items()
#     #               if k[:4] == "bert" and "pooler" not in k}
#
#     checkpoint['bert.embeddings.word_embeddings.weight'] = checkpoint['bert.embeddings.word_embeddings.weight'][keep_tokens]
...
def load_pretrained_bert(model, pretrained_model_path, keep_tokens=None):
    checkpoint = torch.load(pretrained_model_path)
    checkpoint = {k: v for k, v in checkpoint.items() if k[:4] == 'bert'}
    checkpoint['bert.embeddings.word_embeddings.weight'] = checkpoint['bert.embeddings.word_embeddings.weight'][keep_tokens]
...
def load_chinese_base_vocab(vocab_path):
    ...
    for char in special_chars:
        new_token_dict[char] = len(new_token_dict)
        keep_tokens.append(char2idx[char])

    for char, _ in sorted(char2idx.items(), key=lambda s: s[1]):
        if char not in new_token_dict:
            keep = True
            if len(char) > 1:
                for c in Tokenizer.stem(char):
                    if (
...
if __name__ == '__main__':
    char2idx, keep_tokens = load_chinese_base_vocab('./bert/vocab.txt')